...
import { Plus, Trash2, Edit, Check, ChevronDown, ChevronUp, X, RotateCcw, Sparkles, Mic, MicOff, StopCircle } from "lucide-react";
...
export default function LibroDiario() {
  ...
  const updateLinea = (i: number, field: string, value: string | number) =>
      ...
      recorder.onstop = async () => {
        ...
        reader.onloadend = async () => {
          ...
          try {
            const resp = await fetch("https://api.anthropic.com/v1/messages", {
              ...
              body: JSON.stringify({
                ...
                messages: [{
                  role: "user",
                  content: [
                    { type: "text", text: "Transcribí exactamente lo que dice este audio en español. Solo el texto transcrito, sin comentarios." },
            ...
            const transcripcion = d.content?.[0]?.text || "";
            if (transcripcion) setExplicacion(transcripcion);
            else toast.error("No se pudo transcribir el audio");
          } catch {
            toast.error("Error al transcribir");
          } finally {
            setAutoDescribirLoading(false);
          }
        };
  ...
  const handleAutoDescribir = async () => {
    const lineasConCuenta = lineas.filter(l => l.cuenta_id);
    if (lineasConCuenta.length === 0) { toast.error("Seleccioná al menos una cuenta"); return; }
    if (!explicacion && !glosa) { toast.error("Escribí una explicación o la glosa primero"); return; }

    setAutoDescribirLoading(true);
    try {
      const cuentasInfo = lineasConCuenta.map(l => {
        const c = getCuenta(l.cuenta_id);
        const lado = l.debe > 0 ? `Debe: ${l.debe}` : `Haber: ${l.haber}`;
        return `${c?.codigo} - ${c?.nombre} (${c?.tipo}, ${lado})`;
      }).join("\n");

      const contexto = explicacion
        ? `Explicación del usuario: "${explicacion}"${glosa ? `\nGlosa: "${glosa}"` : ""}`
        : `Glosa: "${glosa}"`;
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Sos un contador boliviano experto. El usuario te explicó en lenguaje natural lo que está registrando. Basándote en esa explicación y las cuentas involucradas, generá:
1. Una glosa concisa (máximo 6 palabras) para el comprobante
2. Una descripción corta (máximo 8 palabras) para cada línea contable
...
Respondé SOLO con un JSON objeto con exactamente este formato, sin explicaciones ni markdown:
{"glosa": "texto de la glosa", "descripciones": ["desc línea 1", "desc línea 2"]}

Las descripciones deben ser ${lineasConCuenta.length} en total, en el mismo orden que las cuentas.`
          }]
        })
      });
      ...
      const descripciones: string[] = parsed.descripciones || parsed;
      const nuevaGlosa: string = parsed.glosa || "";

      if (nuevaGlosa && !glosa) setGlosa(nuevaGlosa);
      else if (nuevaGlosa) setGlosa(nuevaGlosa);

      let idx = 0;
      setLineas(prev => prev.map(l => {
        if (!l.cuenta_id) return l;
        const desc = descripciones[idx] || "";
        idx++;
        return { ...l, descripcion: desc };
      }));
      setShowExplicacion(false);
      toast.success("Glosa y descripciones generadas ✓");
    } catch (e) {
  ...
  const cuentasActivas = cuentas
    .filter(c => c.activa)
    .sort((a, b) => {
      // Sort by code naturally: A1.1, A1.2, ..., P1.1, C1.1, I1.1, G1.1
      const parseCode = (code: string) => {
        const match = code.match(/^([A-Z]+)(\d+)\.(\d+)$/);
        if (!match) return [code, 0, 0];
        const prefix = match[1];
        const order: Record<string, number> = { A: 1, P: 2, C: 3, I: 4, G: 5 };
        return [order[prefix] || 9, parseInt(match[2]), parseInt(match[3])];
      };
      const [pa, na, sa] = parseCode(a.codigo) as [number, number, number];
      const [pb, nb, sb] = parseCode(b.codigo) as [number, number, number];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-3">
              <div className="flex items-center justify-between mb-1.5">
                <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground block">Glosa *</Label>
                <button
                  type="button"
                ...
                >
                  ...
                  {showExplicacion ? "Ocultar asistente" : "✨ Generar con IA"}
                </button>
              </div>
              ...
              {showExplicacion && (
                <div className="mt-3 rounded-lg border border-primary/30 bg-primary/5 p-3 space-y-2">
                  <p className="text-xs text-muted-foreground font-medium">
                    Explicá en tus palabras qué estás registrando — la IA generará la glosa y descripciones automáticamente.
                  </p>
                  <textarea
                    className="w-full rounded-md border border-border bg-background text-sm p-2 resize-none focus:outline-none focus:ring-1 focus:ring-primary"
                    rows={3}
                    value={explicacion}
                    onChange={e => setExplicacion(e.target.value)}
                    placeholder="Ej: Estoy pasando plata de mi capital personal a la caja de ahorro para empezar a ahorrar..."
                  />
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={isRecording ? handleStopRecording : handleStartRecording}
                      disabled={autoDescribirLoading}
                    ...
                    >
                      {isRecording ? <><StopCircle className="h-3.5 w-3.5" /> Detener grabación</> : <><Mic className="h-3.5 w-3.5" /> Grabar voz</>}
                    </button>
                    {isRecording && (
                      <span className="text-xs text-destructive animate-pulse font-medium">● Grabando...</span>
                    )}
                    <button
                      type="button"
                      onClick={handleAutoDescribir}
                      disabled={autoDescribirLoading || (!explicacion && !glosa)}
                      className="ml-auto flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    >
                      <Sparkles className="h-3.5 w-3.5" />
                      {autoDescribirLoading ? "Generando..." : "Generar descripciones"}
                    </button>
                  </div>
                </div>
              )}
