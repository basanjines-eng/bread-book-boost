...
import { Plus, Trash2, Edit, Check, ChevronDown, ChevronUp, X, RotateCcw, Sparkles } from "lucide-react";
...
export default function LibroDiario() {
  ...
  const handleAutoDescribir = async () => {
    if (!glosa) { toast.error("Escribí la glosa primero"); return; }
    ...
    try {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `Sos un contador boliviano. Dado este asiento contable, generá una descripción corta y clara (máximo 8 palabras) para cada línea contable.

Glosa del comprobante: "${glosa}"

Cuentas involucradas:
${cuentasInfo}

Respondé SOLO con un JSON array con exactamente ${lineasConCuenta.length} strings, uno por cada cuenta en el mismo orden. Sin explicaciones, sin markdown, solo el JSON array.
Ejemplo: ["Cobro en efectivo al cliente", "Ingreso por venta de productos"]`
          }]
        })
      });
      ...
      const descripciones: string[] = JSON.parse(clean);
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  disabled={autoDescribirLoading || !glosa}
                  className="flex items-center gap-1 text-xs text-primary hover:text-primary/80 disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-medium"
                >
                  <Sparkles className="h-3 w-3" />
                  {autoDescribirLoading ? "Generando..." : "Auto-describir"}
                </button>
              </div>
