...
interface ActivoFijo {
  ...
  depreciacionesRegistradas: number;
}
...
export default function NotasContablesPage() {
  ...
  const obligaciones = useMemo(() => {
    ...
    }).filter(Boolean).filter((o): o is { codigo: string; nombre: string; saldo: number } => Math.abs((o as { saldo: number }).saldo) > 0.01) as { codigo: string; nombre: string; saldo: number }[];
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  <TableRow className="font-semibold">
                    ...
                    <TableCell className="text-right font-mono">{formatMoney(activosFijos.reduce((s: number, a: ActivoFijo) => s + a.costoAdquisicion, 0))}</TableCell>
                    <TableCell></TableCell>
                    <TableCell className="text-right font-mono">
                      {formatMoney(activosFijos.reduce((s: number, a: ActivoFijo) => s + (a.costoAdquisicion / (a.vidaUtilAnios * 12)) * a.depreciacionesRegistradas, 0))}
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {formatMoney(activosFijos.reduce((s: number, a: ActivoFijo) => s + a.costoAdquisicion - (a.costoAdquisicion / (a.vidaUtilAnios * 12)) * a.depreciacionesRegistradas, 0))}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
