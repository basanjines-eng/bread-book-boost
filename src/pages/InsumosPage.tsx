...
export default function InsumosPage() {
  ...
  const openMovForm = (tipo: TipoMovimientoInsumo, insumoId?: string) => {
    ...
    if (insumoId) {
      ...
      if (ins) setMovUnidad(tipo === 'ENTRADA' ? ins.unidad_compra_habitual : ins.unidad_base);
    }
    setShowMovForm(true);
  };
                                          ...
                                          <tbody className="divide-y divide-border/40">
                                            {movs.map(m => (
                                              <tr key={m.id} className="hover:bg-muted/20 transition-colors">
                                                ...
                                                <td className="text-center py-2 px-2">
                                                  <Badge
                                                    variant={m.tipo_movimiento === 'ENTRADA' ? 'default' : m.tipo_movimiento === 'SALIDA' ? 'destructive' : 'secondary'}
                                                    className="text-[10px]"
                                                  >
                                                    {m.tipo_movimiento}
                                                  </Badge>
